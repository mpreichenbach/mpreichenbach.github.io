...
\resheading{Skills}
	\begin{description}
        \item[Python:] { \footnotesize Developed convolutional neural networks using \textbf{Keras} and  the \textbf{Tensorflow} backend to perform land-cover classification of high-resolution imagery; trained models using \textbf{multiple GPUs} on an NVIDIA DGX machine; processed satellite and drone imagery using the \textbf{NumPy}, \textbf{SciPy}, \textbf{OpenCV}, and \textbf{Rasterio} packages to create training datasets; primarily use the \textbf{Anaconda} distribution for current work.}
        
        \item[R:] { \footnotesize Analyzed time-series data of invasive carp movements using the \textbf{tidyverse} and \textbf{ggplot2} libraries; fit Hidden Markov Models with the \textbf{momentuHMM} and \textbf{parallel} libraries; performed spatial interpolation of sound intensity data with the \textbf{raster} and \textbf{automap} libraries, among others.}
        	
		\item[Additional Languages:] {\footnotesize \LaTeX.}
		
		\item[Applications:]{\footnotesize QGIS, ArcGIS Pro, Microsoft Office, Google Docs.}
		
        \item[Operating Systems:] {\footnotesize Windows, Linux, MAC OS}.
...
\resheading{Conference Presentations}
	\begin{etaremune}
		...
		\item
			\ressubheading{ERDC Research and Development Symposium (RD22)}{Virtual}{``Modeling the Effects of Acoustic Deterrents on Invasive Carp Behavior'' (15 mins)}{Mar. 2022}
	\end{etaremune}
	
\resheading{Seminar Presentations}
	\begin{etaremune}
		\item
		\ressubheading{MathBio Seminar (remote), University of Nebraska-Lincoln}{Lincoln, NE}{``Modeling the Effects of Acoustic Deterrents on Invasive Carp Behavior'' (50 mins)}{Mar. 2022}
		\item
